...
--- FROM: Specifies the Supplier table. WHERE: Filters rows where the country is UK. 
---This was arrived at based on simple data retrieval based on a condition.

--- QUESTION 2 
--- List the first name, last name, and city for all customers. 
--- Concatenate the first and last name separated by a space and a comma as a single column
SELECT LastName + ', ' + FirstName AS FullName, City FROM Customer;

--- Concatenation (+): Merges last and first names into one column (FullName).
--- AS FullName: Renames the output column. City: Included for geographic context.

--- QUESTION 3 
--- List all customers in Sweden
SELECT * FROM Customer 
WHERE Country = 'Sweden';
--- Basic filter on the Country column in Customer.
...
---ORDER BY: Sorts the results by CompanyName in ascending order.
...
---JOIN: Connects Supplier and Product by foreign key (SupplierId).
---Alias: s and p used to shorten table names.
...
---Combines orders and customer details. JOIN on CustomerId foreign key. Provides a full picture of each transaction.
...
---Three tables: OrderItem gives line items; Product adds names; "Order" adds numbers. Sorted by OrderNumber.

---QUESTION 8
---Using a case statement, list all the availability of products. When 0 then not available, else available
SELECT ProductName,
CASE WHEN IsDiscontinued = 0 THEN 'Available' ELSE 'Not Available' END AS Availability FROM Product;
---CASE: Logical check.Is Discontinued = 0 → available; 1 → not available. Makes boolean readable to end-users.

---QUESTION 9
---Using case statement, list all the suppliers and the language they speak. 
---The language they speak should be their country E.g if UK, then English
SELECT CompanyName, Country,
CASE WHEN Country = 'UK' THEN 'English'
WHEN Country = 'Germany' THEN 'German'
ELSE 'Unknown'
END AS Language
FROM Supplier;

---Maps each country to a language using CASE logic.
---Provides an inferred attribute not stored in the Database.

---QUESTIOM 10
---List all products that are packaged in Jars
SELECT * FROM Product 
WHERE Package LIKE '%jar%';
---LIKE '%jar%': Pattern matching (case-insensitive depending on DB collation).
---Filters for any package containing “jar”.


---QUESTION 11
---List procucts name, unitprice and packages for products that starts with Ca
SELECT ProductName, UnitPrice, Package FROM Product 
WHERE ProductName LIKE 'Ca%';
---LIKE 'Ca%': Filters product names that start with “Ca”.

---QUESTION 12
---List the number of products for each supplier, sorted high to low.
SELECT s.CompanyName, COUNT(p.Id) AS Product_Count 
FROM Supplier s
JOIN Product p ON s.Id = p.SupplierId 
GROUP BY s.CompanyName 
ORDER BY Product_Count DESC;
---COUNT(): Counts products. GROUP BY: Needed for aggregation. Sorted high to low with ORDER BY DESC.
--QUESTION 13
---List the number of customers in each country.
SELECT Country, COUNT(*) AS Customer_Count 
FROM Customer 
GROUP BY Country;
---Basic aggregation by Country. COUNT(*) counts all customers per country.
...
---Basic aggregation by Country. COUNT(*) counts all customers per country. 
---Sorted in CustomerCount descending order.
...
---SUM(): Totals customer purchases. JOIN: Links orders to customer details. ORDER BY: Shows top spenders first.
...
GROUP BY Country 
HAVING COUNT(*) > 2;
---HAVING Filters aggregated data (after GROUP BY).Shows supplier count by Country.
...
GROUP BY Country 
HAVING COUNT(*) > 10;
---HAVING Filters aggregated data (after GROUP BY).Shows Customer count by Country
...
SELECT c.FirstName + ' ' + c.LastName AS Customer_Name,
AVG(o.TotalAmount) AS Avg_Order_Amount 
FROM "Order" o
JOIN Customer c ON o.CustomerId = c.Id
GROUP BY c.FirstName, c.LastName
HAVING AVG(o.TotalAmount) BETWEEN 1000 AND 1200;
---Uses AVG() to find average order value. HAVING filters based on that average.
...
---BETWEEN: Time-based filter. Tracks activity in a defined period (Jan 2013).
